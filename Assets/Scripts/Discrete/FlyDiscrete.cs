...
public class FlyDiscrete : Agent
{
    public class Snake{
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        if(true){
            ...
            transform.Rotate(transform.up * 90 * horizontalAxis,Space.Self);

            //Rotate vertical
            float verticalAxis = Input.GetAxisRaw("Vertical");
            if(transform.rotation.y == 90 || transform.rotation.y == -90){
                transform.Rotate(transform.right * 90 * verticalAxis, Space.Self);
            }else{
                transform.Rotate(transform.forward * 90 * verticalAxis, Space.Self);
            }
